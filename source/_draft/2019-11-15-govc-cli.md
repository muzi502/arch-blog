---
title: vSphere ESXi 命令行工具 govc 的使用
date: 2019-11-15
link: govc-cli
---
...
此文仅仅针对 VMware vSphere虚拟化、 非 vSphere ESXi 用户可以洗洗睡了😥

不过我觉着 VMware 虚拟化对于中小型企业来讲还是不错滴，毕竟 VMware 他爹是 Dell 嘛😂，所以 vSphere 与 Dell 服务器结合起来才是天衣无缝嘛，毕竟 VMware 对 Dell 服务器下足功夫做了优化。

上上个月的时候，在研究如何在 vSphere 虚拟化平台上搓一套 kubernetes 集群出来，想着如何从底层的虚拟机动手，就发现了 govc 这个工具，而 govc 只是 govmoni 的一部分。
...
下面剽窃一段官方 [README]( https://github.com/vmware/govmomi/blob/master/README.md )，其实下面大半部分也是剽窃翻译的  govc [官方使用文档]( https://github.com/vmware/govmomi/blob/master/govc/USAGE.md ) 😥
...
概括来讲 govmomi 是一个使用 Golang 实现用来与 VMware vSphere APIs 交互的工具库/客户端，这里面包含了 一个 vSphere 命令行工具 govc 。使用 govc 可以来直觉操控 ESXi 主机或者 vCenter。

## 下载和安装

由于是使用 Golang 写的，根据自己的系统类型在官方 GitHub 的 [release]( https://github.com/vmware/govmomi/releases ) 下载下来相应的二进制可执行文件并添加到环境变量即可食用。

##  使用

This document is generated from `govc -h` and `govc $cmd -h` commands.

The following common options are filtered out in this document, but appear via `govc $cmd -h`:

```json
  -cert=                    Certificate [GOVC_CERTIFICATE] # 证书
  -debug=false              Store debug logs [GOVC_DEBUG]  # debug 选项
  -dump=false               Enable output dump
  -json=false               Enable JSON output             # 输出为 json 格式
...
```ini
Certificate Status:          ERROR x509: certificate signed by unknown authority
Issued To:
  Common Name (CN):          10.10.107.188
  Organization (O):          VMware
  Organizational Unit (OU):  VMware Engineering
Issued By:
  Common Name (CN):          CA
  Organization (O):          ICOS-20190425AC
  Organizational Unit (OU):  VMware Engineering
Validity Period:
  Issued On:                 2:08 +0000 UTC
  Expires On:                2:08 +0000 UTC
Thumbprints:
  SHA-256 Thumbprint:        DD:C4:84:EA:3F:4D:C2
  SHA-1 Thumbprint:          D:40:5A:77:88:48:65
