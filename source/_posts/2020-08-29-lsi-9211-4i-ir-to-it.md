...
收到货后，迫不及待地将存满老婆的硬盘接到阵列卡上，开机的时候，一直卡在阵列卡 BIOS 初始化页面很久很久。顿时觉着，完蛋翻车了，正准备去怼一波卖家，开机后就躺床上让他等去吧。没想到过了五分钟之久，阵列卡终于初始化完成，进入了久违的  ESXi 的启动页面。这尼玛，坑我啊，你一个巴掌大的阵列卡启动竟然需要五分钟？内心你一万匹草泥马奔腾而过。

![](img/Eck8sxtVAAEEzfD.png)

## IR 模式？

在网上看来一下大佬们的评论，说阵列卡 IR 模式的启动速度要慢一些？然后又找到了 [IBM H1110卡刷为LSI 9211-4i IT固件](https://wiki2.xbits.net:4430/hardware:lsi:h1110-it-firmware) ，里面提到的可以将 IR 模式刷为 IT 直通模式。
...
>   我青年时代就刷过：诺基亚、摩托罗拉、三星、黑莓、Lumia、，我还刷过：路由器、交换机、阵列卡、N1 盒子

安卓~~基~~机佬们想必对 twrp 再熟悉不过了，它是我们刷机，刷各种 ROM 包最常用的系统，相当于 Windows 中的 PE 系统一样，对系统分区刷上 ROM 来达到刷机的目的。刷阵列卡的固件和刷机也差不多，也是需要一个底层的系统进去对阵列卡上的存储芯片进行操作。最常见的就是 Dos 和 EFI Shell ，由于我的 Dos 年代久远，且我的主板对 EFI 支持比较友好，在进行刷机的时候为了稳妥起见选择 EFI shell。
...
刷完之后阵列卡的型号就莫名其妙地变成了 `LSI® SAS 9211-4i PCI Express® to 6Gb/s Serial Attached SCSI (SAS) Host Bus Adapter` ，固件的版本也变成了  IT 模式，开机的速度比以前快了很多，只需要不到 5s 就自检完成进入 ESXi 的启动流程。

![](img/20200828_203215441_iOS.jpg)

在 ESXi 中，阵列卡也被识别为了 LSI2004 ，看来是没问题了，插上硬盘也都是识别出来了。

![image-20200908085638618](img/image-20200908085638618.png)
